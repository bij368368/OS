...
function renderProducts(target='#productGrid',filter='all'){const el=document.querySelector(target);if(!el)return;const filtered=filter==='all'?products:products.filter(p=>p.type===filter);el.innerHTML=filtered.map(p=>`<article class="card"><a class="card-media" href="/products/${p.slug}.html"><img src="${p.img}" alt="Only Samosas ${p.name} frozen samosa pack" loading="lazy" width="800" height="800"></a><div class="card-body"><span class="badge ${p.type==='meat'?'red':''}">${p.tag}</span><div class="card-title-row"><h3><a class="plain-link" href="/products/${p.slug}.html">${p.id==='lamb'?'spring <strong>LAMB</strong>':p.name}</a></h3></div><p>${p.desc}</p><div class="card-actions"><a class="btn primary" href="/products/${p.slug}.html">Explore flavour</a></div></div></article>`).join('')}
